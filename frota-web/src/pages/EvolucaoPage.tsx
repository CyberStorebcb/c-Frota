...
export function EvolucaoPage() {
  ...
  const xInterval = useMemo(() => {
    if (agregacao === 'mes') return 0
    // Em telas menores, não renderizar TODOS os labels.
    // Objetivo: ~6 labels visíveis no eixo X.
    const target = wrapW && wrapW < 900 ? 5 : 8
    if (chartData.length <= target) return 0
    return Math.max(1, Math.ceil(chartData.length / target) - 1)
  }, [agregacao, wrapW, chartData.length])
        ...
        <div ref={wrapRef} className="mt-4 h-[380px] min-w-0 px-1">
          {resolvidosFiltrados.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0}>
              <ComposedChart data={chartData} margin={{ top: 34, right: 24, bottom: 78, left: 24 }}>
                <Legend
                  verticalAlign="top"
                  align="right"
                ...
                <XAxis
                  ...
                  interval={xInterval}
                  minTickGap={16}
                  height={agregacao === 'mes' ? 42 : 68}
                  angle={agregacao === 'mes' ? 0 : -22}
                ...
                <YAxis
                  ...
                  domain={[
                    0,
                    (max: number) => (Number.isFinite(max) && max > 0 ? Math.ceil(max * 1.15) : 10),
                  ]}
                  label={{
                    value: 'Dias médios',
                    angle: 90,
                ...
                <Line
                  ...
                  stroke="#0EA5E9"
                  strokeWidth={2.5}
                  // No semanal existem muitas semanas vazias (diasMedios=null).
                  // Conectar os pontos evita “sumir” a linha.
                  connectNulls
