...
function _desenharGrafico(
  ...
  const sorted = dados.slice(0, 30)
  const dpr    = 2
  const W      = 860
  const barH   = 30
  const labelW = 230
  const pad    = { top: 72, right: 80, bottom: 36, left: 16 }
  const chartW = W - labelW - pad.right - pad.left
  const H      = pad.top + sorted.length * (barH + 7) + pad.bottom
  ...
  ctx.textAlign = 'left'
  ctx.fillText(titulo, pad.left + labelW, 30)

  // Legenda
  const lx = pad.left + labelW + chartW - 180
  ctx.fillStyle = '#334155'; ctx.beginPath(); ctx.roundRect(lx, 14, 14, 12, 2); ctx.fill()
  ctx.fillStyle = '#94a3b8'; ctx.font = '11px Arial'; ctx.fillText('Total', lx + 18, 24)
  ctx.fillStyle = '#ef4444aa'; ctx.beginPath(); ctx.roundRect(lx + 80, 14, 14, 12, 2); ctx.fill()
  ctx.fillStyle = '#94a3b8'; ctx.fillText('Com NC', lx + 98, 24)

  const maxTotal = Math.max(...sorted.map((d) => d.total), 1)

  sorted.forEach((d, i) => {
    const y    = pad.top + i * (barH + 7)
    const bw   = Math.max(4, (d.total / maxTotal) * chartW)
    const ncW  = d.total > 0 ? (d.nc / d.total) * bw : 0
    ...
    ctx.textAlign = 'right'
    const lbl = d.label.length > 30 ? d.label.slice(0, 29) + '…' : d.label
    ctx.fillText(lbl, x0 - 8, y + barH * 0.66)

    // Barra total
    ctx.fillStyle = '#334155'
    ctx.beginPath(); ctx.roundRect(x0, y, bw, barH, 4); ctx.fill()

    // Barra NC
    if (ncW > 0) {
      ctx.fillStyle = '#ef4444aa'
      ctx.beginPath(); ctx.roundRect(x0, y, ncW, barH, 4); ctx.fill()
    }
    ...
    ctx.textAlign = 'left'
    const pct = d.total > 0 ? ` · ${((d.nc / d.total) * 100).toFixed(0)}% NC` : ''
    ctx.fillText(`${d.total}${pct}`, x0 + bw + 7, y + barH * 0.66)
  })
...
async function exportarExcel(rows: ChecklistRow[]) {
  ...
  ws.columns = [
    { key: 'data',       width: 14 },
    { key: 'tipo',       width: 14 },
    { key: 'operador',   width: 28 },
    { key: 'matricula',  width: 11 },
    { key: 'supervisor', width: 26 },
    { key: 'placa',      width: 10 },
    { key: 'km',         width: 11 },
    { key: 'horimetro',  width: 11 },
    { key: 'modelo',     width: 28 },
    { key: 'prefixo',    width: 28 },
    { key: 'localidade', width: 20 },
    { key: 'processo',   width: 14 },
    { key: 'c',          width:  9 },
    { key: 'nc',         width:  9 },
    { key: 'problemas',  width: 42 },
  ]
  ...
  rows.forEach((r, i) => {
    ...
    // Destaca NC > 0 em vermelho, 0 em verde
    const ncCell = row.getCell(14)
    if (r.nc_count > 0) {
      ncCell.font = { bold: true, color: { argb: 'FFdc2626' }, size: 10, name: 'Calibri' }
      ncCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFfef2f2' } }
  ...
  // Sheet 2 — Gráficos
  // ══════════════════════════════════════════════════════════════
  const ws2 = wb.addWorksheet('Gráficos')
  ...
  // Gráficos como imagens PNG
  const imgBase64Base = desenharGrafico('Checklists por Base / Localidade', porBase)
  const imgBase64Sup  = desenharGrafico('Checklists por Supervisor', porSup)

  const idBase = wb.addImage({ base64: imgBase64Base, extension: 'png' })
  const idSup  = wb.addImage({ base64: imgBase64Sup,  extension: 'png' })

  const imgW    = 820
  const baseH   = Math.max(360, porBase.length * 37 + 108)
  const supH    = Math.max(360, porSup.length  * 37 + 108)
  const gap     = 3

  // Gráfico 1 — Base (começa na linha 0)
  ws2.addImage(idBase, { tl: { col: 0, row: 0 }, ext: { width: imgW, height: baseH } })

  // Gráfico 2 — Supervisor (começa logo abaixo)
  const supStartRow = Math.ceil(baseH / 20) + gap
  ws2.addImage(idSup, { tl: { col: 0, row: supStartRow }, ext: { width: imgW, height: supH } })

  // Tabelas resumo ao lado direito
  const addTabela = (
    startRow: number,
    startCol: number,
    titulo: string,
    ...
    // Título
    const tCell = ws2.getCell(startRow, startCol)
    tCell.value = titulo
    tCell.font  = { bold: true, size: 12, color: { argb: 'FF064e3b' }, name: 'Calibri' }
    tCell.fill  = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFecfdf5' } }
    ws2.getRow(startRow).height = 22

    // Cabeçalho
    const hRow = ws2.getRow(startRow + 1)
    hRow.height = 20
    cabecalho.forEach((h, ci) => {
      const c = hRow.getCell(startCol + ci)
      c.value = h
      c.font  = { bold: true, color: BRANCO, size: 10, name: 'Calibri' }
      c.fill  = { type: 'pattern', pattern: 'solid', fgColor: VERDE_HEADER }
    ...
    dados.forEach((d, i) => {
      ...
      const dRow  = ws2.getRow(rIdx)
      dRow.height = 17
      const bg    = i % 2 === 0 ? BRANCO : CINZA_LINHA
      const pct   = d.total > 0 ? `${((d.nc / d.total) * 100).toFixed(1)}%` : '0%'
      ;[d.label, d.total, d.nc, pct].forEach((v, ci) => {
        const c = dRow.getCell(startCol + ci)
        c.value = v
    ...
    ws2.getColumn(startCol).width     = 32
    ws2.getColumn(startCol + 1).width = 10
    ws2.getColumn(startCol + 2).width = 10
    ws2.getColumn(startCol + 3).width = 10
  }

  addTabela(1, 13, 'Por Base / Localidade', ['Base / Localidade', 'Total', 'Com NC', '% NC'], porBase)
  addTabela(supStartRow + 1, 13, 'Por Supervisor', ['Supervisor', 'Total', 'Com NC', '% NC'], porSup)
  ...
  const a      = document.createElement('a')
  a.href = url
  a.download = `checklists_${new Date().toISOString().slice(0, 10)}.xlsx`
  a.click()
  URL.revokeObjectURL(url)
}
